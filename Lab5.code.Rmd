...
```
Hypothesis on data distribution: The distribution of the preposition type and the distribution of the case marking in Russian are assosiated (not independent).
```

and H<sub>0</sub> hypothesis: 
```
H0 hypothesis: The distribution of the preposition type and the distribution of the case marking in Russian are independent.
```
...
```{r 1_3}
chisq.test(russian.tab)$expected >= 5
```
...
```{r 1_6}
sqrt(1.6459 / ((156 + 95 + 96 + 119) * (min(2,2)-1)))
```
...
[Cohen, J., 1988: 232]
```
If effect size is large, we cannot explain chisq statistics just  by the huge amount of data.

### Mosaic and association plots
```{r}
mosaic(russian.tab, shade = TRUE, varnames = FALSE)
chisq.test(russian.tab)$residuals
chisq.test(russian.tab)$stdres # standardized Pearson residuals (devided by standard error)
...
```
The p-value stat is not in the critical region (which is less than 0.05 here). 
There is insuffiient evidence to reject the H0 in favor of the H1. Thus, the null hypothesis could be true.
```
...
### 3.1 <?> Create a matrix or a data frame
e.g. 
```{r}
x <- (matrix(c(5485, 8318, 15913, 8259), nrow = 2))
```
